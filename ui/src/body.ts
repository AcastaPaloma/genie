...
export class FlyBodyView {
  ...
  private camera = new THREE.PerspectiveCamera(40, 1, 0.01, 1000);
  ...
  constructor(private element: HTMLElement) {
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      "Physically simulated NeuroMechFly body. Joint motion is driven by the neural output adapter.",
    );
    element.append(this.renderer.domElement);
    this.camera.up.set(0, 0, 1);
    this.camera.position.set(-5, -7, 4.5);
    ...
    const floor = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x313b3e, roughness: 1 }),
    );
    floor.position.z = -0.01;
    this.scene.add(floor);
    const grid = new THREE.GridHelper(200, 200, 0x566569, 0x3c474a);
  ...
  async load(onStage: (text: string) => void) {
    this.physics = await loadScene({
      assetsDir: `${import.meta.env.BASE_URL}body/assets`,
      onStage,
    });
  ...
  setTimestep(seconds: number) {
    ...
    const options = this.physics.model.opt;
    options.timestep = seconds;
    this.physics.model.opt = options;
    this.physics.meta.timestep = seconds;
    this.controller = new Controller(this.physics.meta);
  }
  applyPhysicalState(state: {
    ...
    if (!this.physics) return;
    this.dirty = true;
    this.physics.data.qpos.set(state.qpos);
    this.physics.data.geom_xpos.set(state.positions);
    this.physics.data.geom_xmat.set(state.rotations);
    this.physics.data.time = state.time;
  }
  setView(view: string) {
    const directions: Record<string, THREE.Vector3> = {
      angled: new THREE.Vector3(-5, -7, 4.5),
      front: new THREE.Vector3(8, 0, 2.5),
      side: new THREE.Vector3(0, -8, 2.5),
      top: new THREE.Vector3(0, -0.01, 9),
    };
    this.controls.target.copy(this.follow).add(new THREE.Vector3(0, 0, 0.7));
    this.camera.position
      .copy(directions[view] ?? directions.angled)
      .sub(new THREE.Vector3(0, 0, 0.7))
      .multiplyScalar(this.framingScale)
  ...
  step(ms: number, left: number, right: number) {
    ...
    if (!Number.isFinite(data.qpos[0]))
      throw Error("The body simulation became unstable. Reset the experiment.");
  }
  render(onlyIfChanged = false) {
    if (this.physics && this.meshes) {
      syncMeshes(this.meshes, this.physics.data);
      // Follow height as well as ground position: while walking qpos[2] barely moves, but in the worker's air
      // mode the fly floats well above the floor and would otherwise leave the panel from the top.
      const position = new THREE.Vector3(
        this.physics.data.qpos[0],
        this.physics.data.qpos[1],
        this.physics.data.qpos[2],
      );
  ...
  getState() {
    ...
    return {
      ...
      joints: Array.from(data.qpos.slice(7, 49)) as number[],
    };
  }
  reset() {
    if (!this.physics) return;
  ...
  private resize() {
    ...
    this.camera.position
      ...
      .multiplyScalar(scale / this.framingScale)
      .add(this.controls.target);
    this.framingScale = scale;
    this.controls.maxDistance = 22 * scale;
  ...
  dispose() {
    ...
    this.physics?.data.delete();
    this.physics?.model.delete();
